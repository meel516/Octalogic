...
const ConfirmationStep: React.FC<ConfirmationStepProps> = ({ 
  ...
  const handleSubmit = async () => {
    if (!formData.vehicleId || !formData.dateRange.startDate || !formData.dateRange.endDate) {
      return;
    }
      ...
      >
        <Box textAlign="center" p={3}>
          ...
          <Typography variant="body1" paragraph>
            Thank you for your booking, {formData.firstName}! Your rental has been confirmed.
          </Typography>
          ...
          <Box mt={4} p={3} bgcolor="background.paper" borderRadius={2} boxShadow={1}>
            ...
            <Box display="flex" justifyContent="space-between" mb={1}>
              ...
              <Typography variant="body2" fontWeight={500}>
                {dayjs(bookingResponse.booking.startDate).format('MMM D, YYYY')} - {dayjs(bookingResponse.booking.endDate).format('MMM D, YYYY')}
              </Typography>
            </Box>
  ...
  return (
    <Box>
      ...
      >
        ...
        <Box mt={3} p={3} bgcolor="background.paper" borderRadius={2} boxShadow={1}>
          ...
          {vehicle && (
            <>
              ...
              <Box display="flex" justifyContent="space-between" mb={1}>
                ...
                <Typography variant="body2">${vehicle.dailyRate.toFixed(2)}/day</Typography>
              </Box>
            </>
          )}
          ...
          {formData.dateRange.startDate && formData.dateRange.endDate && vehicle && (
            <>
              <Box display="flex" justifyContent="space-between" mb={1}>
                <Typography variant="body2" color="textSecondary">Duration:</Typography>
                <Typography variant="body2">
                  {formData.dateRange.endDate.diff(formData.dateRange.startDate, 'day')} days
                </Typography>
              </Box>
              
              <Box display="flex" justifyContent="space-between" mb={1}>
                <Typography variant="body2" color="textSecondary">Estimated Total:</Typography>
                <Typography variant="body2" fontWeight={700}>
                  ${(formData.dateRange.endDate.diff(formData.dateRange.startDate, 'day') * vehicle.dailyRate).toFixed(2)}
                </Typography>
              </Box>
            </>
          )}
